#!/bin/bash
#SBATCH --job-name=compile_cleo_python_bindings
...
#SBATCH --output=./compile_cleo_python_bindings_out.%j.out
#SBATCH --error=./compile_cleo_python_bindings_err.%j.out
...
cleo_yac_module_path="${path2build}/_deps/cleo-src/libs/coupldyn_yac/cmake"
...
### ----------------- set CLEO flags ------------------- ###
CLEO_YAC_FLAGS="-DCLEO_YAC_MODULE_PATH=${cleo_yac_module_path} \
  -DCLEO_YAXT_ROOT=${yacyaxtroot}/yaxt \
  -DCLEO_YAC_ROOT=${yacyaxtroot}/yac"
